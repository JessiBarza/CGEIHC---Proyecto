...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	/*//AnimPuerta Interacciones que no se implementaron por cuestion de tiempo

	if (keys[GLFW_KEY_SPACE])
	{
		if (rotPuerta == 90.0f || rotPuerta == 0.0f) {
			animacionPuerta = !animacionPuerta;
		}
	}
	if (animacionPuerta)
	{
		while (rotPuerta < 90.0f) {
			if (rotPuerta < 90.0f)
			{
				rotPuerta += 0.5f;
			}
	...
	//Animacion Puerta Horno

	if (keys[GLFW_KEY_H])
	{
		if (rotPuertaH == 90.0f || rotPuertaH == 0.0f) {
			animacionPuertaH = !animacionPuertaH;
		}
	}
	if (animacionPuertaH)
	{
		while (rotPuertaH < 90.0f) {
			if (rotPuertaH < 90.0f)
			{
				rotPuertaH += 0.5f;
			}
			...
			{
				animacionPuertaH = false;
			}
		}
	}
	else {
		while (rotPuertaH > 0.0f)
		{
			if (rotPuertaH > 0.0f)
				rotPuertaH -= 0.5f;
		}
	...
	if (keys[GLFW_KEY_J] && !animacionCajones)
	{
		if ((posCajonIzq == 1.0f || posCajonIzq == 0.0f) && (posCajonDer == 1.0f || posCajonDer == 0.0f)) {
			animacionCajones = true;
	...
	if (animacionCajones)
	{
		float tiempoActual = glfwGetTime() - tiempoInicio;

		// Animación de apertura
		if (tiempoActual <= tiempoAnimacion)
		{
			posCajonIzq = tiempoActual / tiempoAnimacion;
		}
		// Animación de cierre
		else if (tiempoActual <= 2.0f * tiempoAnimacion)
		{
			float tiempoCierre = tiempoActual - tiempoAnimacion;
			posCajonIzq = 1.0f - tiempoCierre / tiempoAnimacion;
		}
		else
		{
			animacionCajones = false;
		}

		// Aplicar la misma lógica para el cajón derecho
		// Animación de apertura

		if (tiempoActual <= tiempoAnimacion)
		{
			posCajonDer = tiempoActual / tiempoAnimacion;
		}
		// Animación de cierre
		else if (tiempoActual <= 2.0f * tiempoAnimacion)
		{
			float tiempoCierre = tiempoActual - tiempoAnimacion;
			posCajonDer = 1.0f - tiempoCierre / tiempoAnimacion;
		}
		else
		{
			animacionCajones = false;
	...
	{
		// Restablecer las variables de posición de los cajones a su estado original
		posCajonIzq = 0.0f;
		posCajonDer = 0.0f;
	}
	...
	if (keys[GLFW_KEY_K])
	{
		if ((rotBandeja == 360.0f || rotBandeja == 0.0f) && bandejaPos == alturaInicial) {
			animacionBandeja = true;
		}
	}
	...
	if (animacionBandeja)
	{
		...
		float alturaActual = bandejaPos - velocidadCaida * deltaTime;
		if (rotBandeja < 360.0f && alturaActual > alturaFinal)
		{
			rotBandeja += velocidadRotacion * deltaTime;
			bandejaPos = alturaActual;
		}
		else
		{
			animacionBandeja = false;
		}
	}*/
